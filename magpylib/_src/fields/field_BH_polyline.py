...
#     mask0[~mask0] = mask1
#     BHJM[~mask0] = (deltaSin / norm_o4 * eB.T / norm_12 * current * 1e-7).T
...
def current_polyline_Bfield(
    *,
    ...
    currents: ndarray, shape (n,)
        Electrical currents in segements.

    Returns
    -------
    B-Field: ndarray, shape (n,3)
        B-field generated by current segements at observer positions.
    ...
    if np.any(mask1):
        ...
        B[~mask1] = (deltaSin / norm_o4 * eB.T / norm_12 * currents * 1e-7).T
        return B
    return (deltaSin / norm_o4 * eB.T / norm_12 * currents * 1e-7).T
...
def BHJM_current_polyline(
    *,
    ...
    BHJM[not_mask0] = current_polyline_Bfield(
        observers=observers,
