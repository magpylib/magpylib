...
def magnet_cylinder_Bfield_axialM(
    *, z0: np.ndarray, r: np.ndarray, z: np.ndarray
) -> list:
    """
    B-field of axially magnetized cylinders.

    The cylinder axis coincides with the z-axis and the geometric center of the
    cylinder lies in the origin. The computation is performed in Cylinder coordinates.
    Length inputs are made dimensionless by division over the cylinder radii.
    Unit polarization is assumed. Implementation based on Derby, American Journal
    of Physics 78.3 (2010): 229-235.

    Parameters
    ----------
    z0: ndarray, shape (n)
        Ratio of cylinder height over cylinder radius.

    r: ndarray, shape (n)
        Ratio of radial observer position over cylinder radius.

    z: Ratio of axial observer position over cylinder radius.

    Returns
    -------
    B-Field: np.ndarray, (Br, Bphi, Bz)
...
def magnet_cylinder_Hfield_diametralM(
    *,
    z0: np.ndarray,
    r: np.ndarray,
    z: np.ndarray,
    ...
    """
    B-field of diametrally magnetized cylinders.

    The cylinder axis coincides with the z-axis and the geometric center of the
    cylinder lies in the origin. The computation is performed in Cylinder coordinates.
    Length inputs are made dimensionless by division over the cylinder radii.
    Unit magnetization is assumed. Implementation partially based on Caciagli:
    Journal of Magnetism and Magnetic Materials 456 (2018): 423-432, and
    [Ortner, Leitner, Rauber] (unpublished).
    ...
    z: ndarray, shape (n)
        Ratio of axial observer position over cylinder radius.

    phi: Azimuth angle between observer and magnetization direction.

    Returns
    -------
    H-Field: np.ndarray, (Hr, Hphi, Hz)
        H-field generated by Cylinders at observer positions in units of A/m.
    """
...
def BHJM_magnet_cylinder(
    *,
    ...
    if any(mask_pol_tv):
        ...
        # Br_tv, Bphi_tv, Bz_tv = magnet_cylinder_Hfield_diametralM(
        #     z0=z0[mask_pol_tv],
        #     r=r[mask_pol_tv],
        #     z=z[mask_pol_tv],
    ...
    if any(mask_pol_ax):
        # Br_ax, _, Bz_ax = magnet_cylinder_Bfield_axialM(
        #     z0=z0[mask_pol_ax], r=r[mask_pol_ax], z=z[mask_pol_ax]
        # )
