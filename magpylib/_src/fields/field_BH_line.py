...
def current_line_Bfield(
    ...
    current: ndarray, shape (n,)
        Electrical current in units of [mm].
    ...
    # characteristic length scale (segment length)
    #p1p2 = p1-p2
    norm_12 = norm(p1-p2, axis=1) # no 0-segments left

    # make dimensionless to avoid all large/small input problems
    p1 = (p1.T/norm_12).T
    p2 = (p2.T/norm_12).T
    po = (po.T/norm_12).T
