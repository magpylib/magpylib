...
def getBH_level1(**kwargs:dict) -> np.ndarray:
    ...
    if src_type == 'Cuboid':
        ...
        B = magnet_cuboid_field(mag, dim, pos_rel_rot, bh)
        ...
        mag = kwargs['magnetization']
        dim = kwargs['dimension']
        B = magnet_cylinder_section_field(mag, dim, pos_rel_rot, bh)

    elif src_type == 'Sphere':
        mag = kwargs['magnetization']
        dia = kwargs['diameter']
        B = magnet_sphere_field(mag, dia, pos_rel_rot, bh)

    elif src_type == 'Dipole':
        moment = kwargs['moment']
        B = dipole_field(moment, pos_rel_rot, bh)

    elif src_type == 'Loop':
        current = kwargs['current']
        dia = kwargs['diameter']
        B = current_loop_field(current, dia, pos_rel_rot, bh)
        ...
        if 'vertices' in kwargs:
            ...
            B = current_line_field(current, pos_start, pos_end, pos_rel_rot, bh)
