...
class RCS:
    ...
    def getB(self,*pos,multicore=False,processes=Max,workers=0): 
        """
        ...
        *pos : vec3 [mm]
        ...
        """
        assert type(pos[0])==list or type(pos[0])==tuple, "Position vector in getB is ill defined."

        results = []

        if multicore is True:
            from multiprocessing import Pool,cpu_count
            if len(pos)==1 and type(pos[0][0])==list or type(pos[0][0])==tuple:
                pos = pos[0]    ## Treat input for Lists of Lists inside of the *pos Tuple
                                ## i.e. getB([[1,1,1],[2,2,2]])
                                ## so it also supports getB([1,1,1],[2,2,2])
            if workers == 0:
                workers = cpu_count() - 1   ## Identify how many workers the host machine can take. 
                                            ## Using all cores is USUALLY a bad idea.
            assert workers > 0, "Could not identify multiple cores for getB. This machine may not support multiprocessing."
