...
def Bfield_Box(MAG, pos, dim): #returns arr3 of field vector in [mT], input in [mT] and [mm]
    ...
    if CASE == 1:
        if abs(x)<=a:
            if abs(y)<=b:
                if abs(z)<=c:
                    print('Warning: Position directly on magnet surface - unphysical solution')
    ...
    if  CASE > 1: 
        ...
        if all([abs(x)<=a,abs(y)<=b,abs(z)<=c]): #directly on magnet edge or corner - log singularity here as result of unphysical model
            print('Warning: Position directly on magnet edge or corner - unphysical solution')
            return array([NaN,NaN,NaN])
        ...
        if   caseA: xma,xpa = -xma,-xpa
        elif caseB: ymb,ypb = -ymb,-ypb
        elif caseC: zmc,zpc = -zmc,-zpc
            
        LOGx = log(((xma+MMM)*(xpa+PPM)*(xpa+PMP)*(xma+MPP)) / ((xpa+PMM)*(xma+MPM)*(xma+MMP)*(xpa+PPP)))
        LOGy = log(((-ymb+MMM)*(-ypb+PPM)*(-ymb+PMP)*(-ypb+MPP)) / ((-ymb+PMM)*(-ypb+MPM)*(ymb-MMP)*(ypb-PPP)))
        LOGz = log(((-zmc+MMM)*(-zmc+PPM)*(-zpc+PMP)*(-zpc+MPP)) / ((-zmc+PMM)*(zmc-MPM)*(-zpc+MMP)*(zpc-PPP)))
        
        if   caseA:  LOGx,xma,xpa = -LOGx,-xma,-xpa
        elif caseB:  LOGy,ymb,ypb = -LOGy,-ymb,-ypb
        elif caseC:  LOGz,zmc,zpc = -LOGz,-zmc,-zpc
    ...
    if xma == 0:
        BxX =  MAGx*(-arctan((ymb*zmc)/(xpa*PMM)) + arctan((ypb*zmc)/(xpa*PPM)) + arctan((ymb*zpc)/(xpa*PMP)) - arctan((ypb*zpc)/(xpa*PPP)) + (pi/2)*(sign(ymb)-sign(ypb))*(sign(zmc)-sign(zpc)))
    elif xpa == 0:
        BxX = -MAGx*(-arctan((ymb*zmc)/(xma*MMM)) + arctan((ypb*zmc)/(xma*MPM)) + arctan((ymb*zpc)/(xma*MMP)) - arctan((ypb*zpc)/(xma*MPP)) + (pi/2)*(sign(ymb)-sign(ypb))*(sign(zmc)-sign(zpc)))
    else:
    ...
    if ymb == 0:
        ByY =  MAGy*(-arctan((xma*zmc)/(MPM*ypb)) + arctan((xpa*zmc)/(PPM*ypb)) + arctan((xma*zpc)/(MPP*ypb)) - arctan((xpa*zpc)/(PPP*ypb)) + (pi/2)*(sign(xma)-sign(xpa))*(sign(zmc)-sign(zpc)))
    elif ypb == 0:
        ByY = -MAGy*(-arctan((xma*zmc)/(MMM*ymb)) + arctan((xpa*zmc)/(PMM*ymb)) + arctan((xma*zpc)/(MMP*ymb)) - arctan((xpa*zpc)/(PMP*ymb)) + (pi/2)*(sign(xma)-sign(xpa))*(sign(zmc)-sign(zpc)))
    else:
    ...
    if zmc == 0:
        BzZ =  MAGz*(-arctan((xma*ymb)/(MMP*zpc)) + arctan((xpa*ymb)/(PMP*zpc)) + arctan((xma*ypb)/(MPP*zpc)) - arctan((xpa*ypb)/(PPP*zpc)) + (pi/2)*(sign(xma)-sign(xpa))*(sign(ymb)-sign(ypb)))
    elif zpc == 0:
        BzZ = -MAGz*(-arctan((xma*ymb)/(MMM*zmc)) + arctan((xpa*ymb)/(PMM*zmc)) + arctan((xma*ypb)/(MPM*zmc)) - arctan((xpa*ypb)/(PPM*zmc)) + (pi/2)*(sign(xma)-sign(xpa))*(sign(ymb)-sign(ypb)))
    else:
