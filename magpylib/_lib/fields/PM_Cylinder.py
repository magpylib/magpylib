...
def Bfield_Cylinder(MAG, pos, dim, Nphi0): #returns arr3
    ...
    #axial field
    Bz_Z = B0z*R/(Rpr)*(betP*elliptic(kP,gamma**2,1,gamma)-betM*elliptic(kM,gamma**2,1,gamma))/pi
    ...
    if B0xy > 0:
        ...
        Br_XY = B0xy*R/2/Nphi0*sum([
            sum([
                (-1)**(k+1)*Sphi(n,Nphi0)*cos(phi0s*n)*(r-R*cos(phi-phi0s*n))*I1x(phi0s*n,z0)
                for z0,k in zip([-H/2,H/2],[1,2])])
            for n in arange(Nphi0+1)])
        #angular component
        Bphi_XY = B0xy*R**2/2/Nphi0*sum([
            sum([
                (-1)**(k+1)*Sphi(n,Nphi0)*cos(phi0s*n)*sin(phi-phi0s*n)*I1x(phi0s*n,z0)
                for z0,k in zip([-H/2,H/2],[1,2])])
            for n in arange(Nphi0+1)])
        #axial component
        Bz_XY = B0xy*R/2/Nphi0*sum([
            sum([
                (-1)**k*Sphi(n,Nphi0)*cos(phi0s*n)/sqrt(r2pR2-rR2*cos(phi-phi0s*n)+(z-z0)**2)
                for z0,k in zip([-H/2,H/2],[1,2])])
            for n in arange(Nphi0+1)])
