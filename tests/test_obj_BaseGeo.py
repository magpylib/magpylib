...
def test_BaseGeo_basics():
    ...
    bgeo.rotate(angle=45, axis=(1, 2, 3))
    poss += [bgeo.position.copy()]
    rots += [bgeo.orientation.as_rotvec()]

    bgeo.rotate(-np.pi / 4, (1, 2, 3), degrees=False)
    poss += [bgeo.position.copy()]
    rots += [bgeo.orientation.as_rotvec()]

    rot = R.from_rotvec((0.1, 0.2, 0.3))
    bgeo.rotate(rot, anchor=(3, 2, 1)).rotate(33, (3, 2, 1), anchor=0)
    poss += [bgeo.position.copy()]
    rots += [bgeo.orientation.as_rotvec()]
...
def test_path_functionality1():
    ...
    pos, ori = pos_orient(BaseGeo(pos0, rot0))
    P = np.array([b1, b2, b3, b4, b5])
    Q = np.array([q1, q2, q3, q4, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)

    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=0))
    P = np.array([b1 + c1, b2 + c2, b3 + c3, b4, b5])
    Q = np.array([q1, q2, q3, q4, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)

    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=1))
    P = np.array([b1, b2 + c1, b3 + c2, b4 + c3, b5])
    Q = np.array([q1, q2, q3, q4, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)

    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=2))
...
def test_path_functionality2():
    ...
    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=3))
    P = np.array([b1, b2, b3, b4 + c1, b5 + c2, b5 + c3])
    Q = np.array([q1, q2, q3, q4, q5, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)

    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=4))
    P = np.array([b1, b2, b3, b4, b5 + c1, b5 + c2, b5 + c3])
    Q = np.array([q1, q2, q3, q4, q5, q5, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)

    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=5))
    P = np.array([b1, b2, b3, b4, b5, b5 + c1, b5 + c2, b5 + c3])
    Q = np.array([q1, q2, q3, q4, q5, q5, q5, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)

    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=6))
    P = np.array([b1, b2, b3, b4, b5, b5 + c1, b5 + c2, b5 + c3])
    Q = np.array([q1, q2, q3, q4, q5, q5, q5, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)

    pos, ori = pos_orient(BaseGeo(pos0, rot0).move(inpath, start="append"))
    P = np.array([b1, b2, b3, b4, b5, b5 + c1, b5 + c2, b5 + c3])
    Q = np.array([q1, q2, q3, q4, q5, q5, q5, q5])
    assert np.allclose(pos, P)
    assert np.allclose(ori, Q)
...
def test_path_functionality3():
    ...
    pos1, ori1 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=4))
    pos2, ori2 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=-1))
    assert np.allclose(pos1, pos2)
    assert np.allclose(ori1, ori2)

    pos1, ori1 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=3))
    pos2, ori2 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=-2))
    assert np.allclose(pos1, pos2)
    assert np.allclose(ori1, ori2)

    pos1, ori1 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=2))
    pos2, ori2 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=-3))
    assert np.allclose(pos1, pos2)
    assert np.allclose(ori1, ori2)

    pos1, ori1 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=1))
    pos2, ori2 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=-4))
    assert np.allclose(pos1, pos2)
    assert np.allclose(ori1, ori2)

    pos1, ori1 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=0))
    pos2, ori2 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=-5))
    assert np.allclose(pos1, pos2)
    assert np.allclose(ori1, ori2)

    pos1, ori1 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=-6))
    pos2, ori2 = pos_orient(BaseGeo(pos0, rot0).move(inpath, start=-5))
    assert np.allclose(pos1, pos2)
    assert np.allclose(ori1, ori2)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "cube().rotate(45,'z').orientation.as_rotvec()",
            np.array([0.0, 0.0, 0.78539816]),
        ),
        (
            "cube().rotate(45, (0,0,1)).orientation.as_rotvec()",
            np.array([0.0, 0.0, 0.78539816]),
        ),
        (
            "cube().rotate(angle=45,axis='z').orientation.as_rotvec()",
            np.array([0.0, 0.0, 0.78539816]),
        ),
        (
            "cube().rotate(rot).orientation.as_rotvec()",
            np.array([0.0, 0.0, 0.78539816]),
        ),
        (
            "cube().rotate(45, axis=(0,0,1)).orientation.as_rotvec()",
            np.array([0.0, 0.0, 0.78539816]),
        ),
        (
            "cube().rotate((0,0,1), angle=45).orientation.as_rotvec()",
            np.array([0.0, 0.0, 0.78539816]),
        ),
    ],
)
def test_rotate_pass_inputs(test_input, expected):
    rot = R.from_rotvec([0, 0, 45], degrees=True)
    cube = lambda : magpy.magnet.Cuboid((0,0,1),(1,1,1))
    assert  np.allclose(eval(test_input), expected)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "cube().rotate(rot, angle=45).orientation.as_rotvec()",
            pytest.raises(MagpylibBadUserInput),
        ),
        (
            "cube().rotate(rot, axis=(0,0,1)).orientation.as_rotvec()",
            pytest.raises(MagpylibBadUserInput),
        ),
        (
            "cube().rotate((0,0,1), 45).orientation.as_rotvec()",
            pytest.raises(MagpylibBadUserInput),
        ),
        (
            "cube().rotate(45, (0,0,1), angle=45).orientation.as_rotvec()",
            pytest.raises(MagpylibBadUserInput),
        ),
        (
            "cube().rotate(45, (0,0,1), axis=(0,0,1)).orientation.as_rotvec()",
            pytest.raises(MagpylibBadUserInput),
        ),
    ],
)
def test_rotate_bad_inputs(test_input, expected):
    rot = R.from_rotvec([0, 0, 45], degrees=True)
    cube = lambda : magpy.magnet.Cuboid((0,0,1),(1,1,1))
    with expected:
