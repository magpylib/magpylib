...
def test_core_phys_moment_of_current_square():
    ...
    H1 = magpy.core.current_polyline_field(
        field="H",
        observers=obs4,
        segment_start=vert[:-1],
        segment_end=vert[1:],
...
def test_core_phys_circle_polyline():
    ...
    B2 = magpy.core.current_polyline_field(
        field="B",
        observers=obs99,
        segment_start=vert[:-1],
        segment_end=vert[1:],
...
def test_core_physics_long_solenoid():
    ...
    # test with solenoid constructed from circle fields
    ...
    Hz_sol = np.sum(H, axis=0)[2]
    np.testing.assert_allclose(Hz_sol, Hz_long, rtol=1e-3)

    # test with cylinder field (using current replacement)
    Jz = MU0 * I * N / L  # polarization <> current density
    ...
    np.testing.assert_allclose(Hz_cyl, Hz_long, rtol=1e-3)
...
def test_core_physics_geometry_cylinder_from_segments():
    ...
    for phi1, phi2 in zip(sections[:-1], sections[1:]):
        B_part = magpy.core.magnet_cylinder_segment_field(
            field="B",
            observers=obs,
...
def test_core_physics_dipole_approximation_magnet_far_field():
    ...
    vol = 1 / 6 * 1e-3
    pol = mom / vol * MU0
    Btetra = magpy.core.magnet_tetrahedron_field(
        field="H",
        observers=obs,
        vertices=vert,
        polarization=pol,
    ...
    pol = mom / vol * MU0
    Bcys = magpy.core.magnet_cylinder_segment_field(
        field="H",
