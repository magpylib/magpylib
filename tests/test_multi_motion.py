...
def test_multi_motion():
    """test multi_motion with context manager
    """
    pm = mag3.magnet.Box((1,2,3),(1,2,3))

    with mag3.multi_motion(pm, steps=333):
        pm.move_to((10,0,0))
    ...
    assert np.allclose(B1,B2), 'simple multi_motion gone wrong'


def test_multi_motion_collection():
