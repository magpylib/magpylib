...
Then top level function `displaySystem(c)` can be used to quickly check the geometry of the sources-sensor-marker assembly. Here `c` can be a source/sensor, a list thereof or a collection. `displaySystem`uses the matplotlib package and its limited capabilities of 3D plotting which often results in bad object overlapping.
...
The following example code shows how a simple collection is initialized and displayed.
...
**Figure:** Output of the above code demonstrating the `displaySystem()` function.
