...
```eval_rst
.. note::
  It is critical to note that the analytical solution does not treat interaction between the sources. This means that the total magnetic field is simply given by the superposition of the field of all sources which can be evaluated individually.
```
...
The idea behind the collection class is to group multiple source objects for common evaluation and manipulation. In principle a collection `c` is simply a list of source objects that are collected in the variable `c.sources`. All methods for geometric operations (`setPosition`, `move`, `setOrientation` and `rotate`) and for calculating the magnetic field (`getB` and `getBsweep`) are also methods of the collection class. A geometric operation applied to a collection is applied to each object within that collection individually. The methods for calculating the magnetic field return the total magnetic field that is generated by the whole collection.
...
In addition, the collection class features methods to add and remove sources for command line like manipulation. The method `c.addSources(*sources)` will add all sources given to it to the collection `c`. The method `c.removeSource(ref)` will remove the referenced source from the collection. Here the `ref` argument can be either a source or an integer indicating the reference position in the collection.
...
c.displaySystem(markers=[(0,0,6),(10,10,10),(-10,-10,-10)])
```
```eval_rst
