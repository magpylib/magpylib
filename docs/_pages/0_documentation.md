...
The idea behind magpylib is to provide simple and easy to use classes and
functions for calculating magnetic fields. The core of the library is the :mod:`~magpylib.source` class which can represent permanent magnets, current distributions or magnetic moments. The library provides simple ways to generate such source objects, to manipulate them geometrically, to group several sources into a :class:`~magpylib.Collection` and to calculate the fields of such systems.
...
- [Library Documentation](#library-documentation)
  ...
  - [Display System Graphically](#display-system-graphically)
...
Once a source object `s` is defined one can calculate the magnetic field generated by it using the methods `getB`. Here the call `s.getB(pos)` simply returns the value of the field which is generated by the source `s` at the sensor position `pos`.
...
In most cases, however, one will be interested to determine the field for a set of sensor positions, or for different magnet positions and orientations. While this can manually be achieved by looping `getB`, the library
offers high-performance methods in vectorized form.

Details will be presented shortly.

Perfomrance tests as well.
...
For calculating the magnetic field that is generated by a whole collection the method `getB` is also available. Just as for geometric operations these methods will be applied to all sources individually, and the total magnetic field will be given by the sum of all parts. A simple example with a collection can be found [here](../2_guideExamples#basic-functionality:-the-field-of-a-collection).
...
Finally, the library includes the top-level function `displaySystem(c)` where `c` can be a source or a collection to quickly check the geometry of the sources-sensor-marker assembly. It uses the matplotlib package and its limited capabilities of 3D plotting which often results in bad object overlapping.

`displaySystem(c)` comes with three keyword arguments:
...
All of these functions are also available in their vectorized versions for quick source definition, so that
a looped definition does not become the bottleneck. This will be detailed in the near future.
