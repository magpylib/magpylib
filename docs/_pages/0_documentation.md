...
There are several examples dedicated to geometrical manipulation in the example section: [Translation, Orientation and Rotation Basics](../2_guideExamples.html#translation-orientation-and-rotation-basics), [Magnet Motion: Simulating a Magnetic Joystick](../2_guideExamples.html#magnet-motion-simulating-a-magnetic-joystick)
...
Geometric addition and subtration operations can be the result when the magnetization vectors are opposed to each other which allows to "cut out" a small volume from a larger one. An [example of a hollow cylinder](../2_guideExamples.html#complex-magnet-shapes-hollow-cylinder) is given in the examples section.
