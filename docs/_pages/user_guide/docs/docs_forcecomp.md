...
where F is the force in Newton and T is the torque in Newton*meter acting on the `targets` via the magnetic field generated by the `sources`. F and T are NumPy arrays of shape (s, p, t, 3), where s is the number of sources, p is the path length, and t is the number of targets.
