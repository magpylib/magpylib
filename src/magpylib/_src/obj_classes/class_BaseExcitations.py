...
class BaseSource(BaseGeo, BaseDisplayRepr):
    ...
    def getB(
        ...
        """Compute the B-field at observers in units of T generated by the source. SI units are
        used for all inputs and outputs.

        Parameters
        ----------
        observers: array-like or Sensor
            Can be array-like positions of shape (n1, n2, ..., 3) where the field
        ...
        squeeze: bool, optional
            Default ``squeeze=True`` squeezes the output, i.e. all axes of length 1 in the output (e.g.
            only a single source) are eliminated.
        pixel_agg: None or str, optional
            ...
            Default is ``None``.
        output: str, optional
            Output type, which must be one of ``'ndarray'`` (default) or  ``'dataframe'``. By
            default a multi-dimensional array is returned. If dataframe is chosen, the function
        ...
        in_out: str, optional
            ...
            - ``'outside'``: All observers are considered to be outside the cuboid; use this for
              performance optimization if applicable.
            Choosing ``'auto'`` is fail-safe but may be computationally intensive.

        Returns
        -------
        B-field: ndarray or DataFrame
            B-field with shape squeeze(m, k, n1, n2, ..., 3) at each path position (index m) for
            each sensor (index k) and each sensor pixel position (indices n1,n2,...) in units
            of T. Sensor pixel positions are equivalent to simple observer positions. Paths of
            objects that are shorter than index m will be considered as static beyond their end.
    ...
    def getH(
        ...
        """Compute the H-field at observers in units of A/m generated by the source. SI units are
        used for all inputs and outputs.
        ...
        Returns
        -------
        H-field: ndarray or DataFrame
            H-field with shape squeeze(m, k, n1, n2, ..., 3) at each path position (index m) for
            each sensor (index k) and each sensor pixel position (indices n1,n2,...) in units
            of T. Sensor pixel positions are equivalent to simple observer positions. Paths of
            objects that are shorter than index m will be considered as static beyond their end.
    ...
    def getM(
        ...
        """Compute magnetization M at observers in units of A/m generated by the source. SI units are
        used for all inputs and outputs.
        ...
        Returns
        -------
        magnetization: ndarray or DataFrame
            Magnetization with shape squeeze(m, k, n1, n2, ..., 3) at each path position (index m) for
            each sensor (index k) and each sensor pixel position (indices n1,n2,...) in units
            of T. Sensor pixel positions are equivalent to simple observer positions. Paths of
            objects that are shorter than index m will be considered as static beyond their end.
    ...
    def getJ(
        ...
        """Compute magnetic polarization J at observers in units of T generated by the source.
        ...
        Returns
        -------
        polarization: ndarray or DataFrame
            Magnetic polarization J with shape squeeze(m, k, n1, n2, ..., 3) at each path position (index m) for
            each sensor (index k) and each sensor pixel position (indices n1,n2,...) in units
            of T. Sensor pixel positions are equivalent to simple observer positions. Paths of
            objects that are shorter than index m will be considered as static beyond their end.
