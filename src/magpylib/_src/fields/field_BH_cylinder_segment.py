...
def magnet_cylinder_segment_Hfield(
    ...
    for cid, cfkt, cargs in zip(case_id, case_fkt, case_args, strict=False):
        ...
        mask = cases == cid
        mask = mask
        args = tuple(allargs[aid] for aid in cargs)
        # result, mask, *args = tuple(xp.broadcast_arrays(result, mask, *args))
        # result = xpx.at(result)[mask].set()
        args = tuple(xp.where(mask, x, xp.nan) for x in args)
        # calc = cfkt(xp, *args)
        H_r = xpx.apply_where(
            mask, args, lambda *args: cfkt(xp, *args), fill_value=xp.nan
        )
        # result = xp.where(mask, calc, result)
