...
def current_vertices_field(
    ...
    """
    xp = array_namespace(observers, current, vertices, segment_start, segment_end)
    ...
    if all(v == nvs[0] for v in nvs):  # if all vertices sets have the same lengths
        ...
        BH = BHJM_current_polyline(
            ...
            segment_start=xp.concatenate([vert[:-1] for vert in vertices]),
            segment_end=xp.concatenate([vert[1:] for vert in vertices]),
        )
